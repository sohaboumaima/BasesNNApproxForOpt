...
#                data.set_value(gr.index, perf_meas, gr[perf_meas].min()[0] / gr[perf_meas])
    ...
    return unique_taus, solver_taus, solvers, data, max_ratio

def draw_simple_pp(taus, solver_vals, solvers, title, max_ratio, filename=None, linestyles=None):
    ...
    folder_path = "performance_profiles"
    ...
    for n, solver in enumerate(solvers):
        if linestyles:
            ke = list(linestyles.keys())
            solver = ke[n]
            ln = linestyles[solver]   
        else:
            ln = '-' 
        ax.step(taus, solver_vals[n, :], label=solver, linestyle=ln)

    # i = 0
    # for solver in linestyles:
    #     n = order[i]

    #     ax.step(taus, solver_vals[n, :], label=solver, linestyle=linestyles[solver])
    #     i += 1
    ...
    # plt.show()
    if filename:
        figure_filename = os.path.join(folder_path, filename)
